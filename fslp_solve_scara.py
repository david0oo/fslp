...
iter_feas = np.array(m_k).squeeze()
plt.semilogy(iters, iter_feas, label=r'Not fully determined NLP, $\varepsilon =-0.06$', marker='.')
...
iters = np.arange(0, len(m_k))
iter_feas = np.array(m_k).squeeze()
plt.semilogy(feasible_solver.list_times, feasible_solver.list_feas, label=r'FSLP', marker='.')
plt.semilogy(ipopt_times, ipopt_inf_pr, label=r'IPOPT', marker='.')
# plt.xlabel('time [s]')
