...
class scara_problem:
    ...
    def __init__(self, N=20):
        ...
        self.n_x = 4 #self.dynamics.size1_in(0)
        self.n_z = 0 #self.dynamics.size1_in(1)
        self.n_u = 2 #self.dynamics.size1_in(2)
        self.n_sh = 3
        self.n_t = 1
        self.n_s0 = self.n_x
        self.n_sf = self.n_x# - 2  # why?????

        # load casadi functions for cartesian to joint conversions and vice
        # versa
    ...
    def create_integrator_time_optimal(self):
        ...
        """
        x = cs.MX.sym('x', self.n_x)  # cs.vertcat(x1, x2, xd1, xd2)
        # z = cs.MX.sym('z', self.n_z)
        u = cs.MX.sym('u', self.n_u)  # cs.vertcat(xdd1, xdd2)

        xdot_temp = cs.vertcat(x[2], x[3], u[0], u[1])#self.dynamics(x, [], u)[0]
        T = cs.MX.sym('T')
    ...
    def create_problem(self, dev_lx_0=[0.0, 0.0], dev_lx_f=[0.0, 0.0]):
        ...
        # qu_end = q_end[iqu]
        self.end_state = cs.vertcat(settings['p_end'], cs.DM.zeros(2, 1))
        # self.end_state = cs.vertcat(qu_end, cs.DM.zeros(nqu, 1))
        # qv_end = q_end[iqv]

        # Define x0, compare with matlab line 62
        self.start_state = cs.vertcat(settings['p_start'], cs.DM.zeros(2, 1))
        ...
        for k in range(self.N+1):
            ...
            # q = self.inverse_kin_fun(cs.vertcat(xee,yee),cs.vertcat(xee_dot,yee_dot))[0]
        ...
        x0_init = cs.vertcat(p_start, cs.DM.zeros(2, 1))
