...
class scara_problem:
    ...
    def create_problem(self, dev_lx_0=np.array([0.0, 0.0]), dev_lx_f=np.array([0.0, 0.0]), obstacleAvoidance=True):
        ...
        for k in range(self.N+1):
            ...
            if k > 0:
                ...
                # self.opti.subject_to(self.opti.bounded(-V_max, xee_dot+yee_dot, V_max))
                # self.opti.subject_to(self.opti.bounded(-V_max, xee_dot-yee_dot, V_max))
        ...
        # self.V_mats = cs.vertcat(cs.sqrt(10)*1e2*S0,
        #                          cs.sqrt(10)*1e2*Sf,
        #                          T)
    ...
    def create_scaling_matrices(self, states=True, controls=True,
                                time=True, sep_hyp=False, slack0=False, slack_f=False, obstacleAvoidance=True):
        """
    ...
    def plot_trajectory(self, list_X, list_labels):
        ...
        if self.obstacleAvoidance:
            ...
            ax.add_patch(rect)
        for i in range(len(list_X)):
            ax.plot(np.array(list_xee_sol[i]).squeeze(), np.array(list_yee_sol[i]).squeeze(), label=list_labels[i], marker='o')
